...
zokou({
  ...
  try {
    ...
    const cap = `*Screenshot taken by DARK MD`;
...
zokou({
  ...
  try {
    // Caption for the screenshot
    const cap = `*Screenshot take by DARK MD`;
...
zokou({
  ...
  try {
    if (!args || args.length === 0) {
      return sendMessage("Please insert the image URL and DARK MD will extract the text for you.");
    }
...
zokou({
  ...
  try {
    if (!args || args.length === 0) {
      return sendMessage("Please describe your image and DARK-MD will generate it.");
    }
    const prompt = args.join(" ");
