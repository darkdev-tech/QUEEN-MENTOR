...
const session = conf.session.replace(/DARK-MD;;;=>/g, "");
    ...
    zk.ev.on("call", async (callData) => {
      if (conf.ANTICALL === 'yes') {
        ...
        await zk.sendMessage(callerId, {
          text: "❗📵I AM DARK MD | I REJECT THIS CALL BECAUSE MY OWNER IS BUSY. KINDLY SEND TEXT INSTEAD."
        });
      }
    });
    ...
    zk.ev.on("messages.upsert", async (m) => {
      ...
      if (ms.message.protocolMessage && ms.message.protocolMessage.type === 0) {
        ...
        if (deletedMessage) {
          ...
          let notification = `*😈DARK ANTIDELETE👿*`;
